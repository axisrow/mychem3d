...
def action1(space:Space):
    ...
    if space.t==600:
        space.compute2atoms()
        #ami = space.merge_from_file("examples/simple/aminogroup.json",x,y,z)
        #space.appendatom(a1)
    ...
    if space.t==2800:
        space.compute2atoms()
        bond_atoms(n1,a1)
        space.atoms[i3+2].color=(0,0,0)
        space.atoms2compute()


    if space.t==4800:
        space.compute2atoms()
        i4 = space.merge_from_file("examples/simple/OH.json",60+x,30+y,z)
        space.atoms[i3+2].nodes[0].q=0
        space.atoms2compute()


    if space.t==5200:
        space.compute2atoms()
        space.atoms[i4].nodes[1].q=1
        space.atoms2compute()
