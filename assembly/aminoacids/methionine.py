...
def action1(space:Space):
    (x,y,z)=(500,500,500)
    global i0,i1,i2,i3,i4,a1,a2,a3,s1
    if space.t==1:    
    ...
    if space.t==1600:
        space.compute2atoms()
        s1 = Atom(550,500,500,6)
        space.appendatom(s1)
        bond_atoms(s1,space.atoms[i3])
        space.atoms2compute()

    if space.t==2000:
        space.compute2atoms()
        i5 = space.merge_from_file("examples/simple/methyl.json",80+x,y,z)
        bond_atoms(space.atoms[i5],s1)  #L/D
        space.atoms2compute()
