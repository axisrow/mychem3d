...
def action1(space):
    ...
    if space.t==1:    # methyl + methyl
        ...
        i1 = space.merge_from_file("examples/nucleobase/guanine.json",90,60,0)
        bond_atoms(space.atoms[i0],space.atoms[i1])
        #space.atoms[i0+1].color = (0,1,0)
        #space.atoms[i1].color = (0,1,0)

        space.atoms[i0+1].nodes[1].q=0
        space.atoms[i1+7].nodes[1].q=0
        space.atoms2compute()
        index = 0

    if space.t==300:
        space.compute2atoms()
        bond_atoms(space.atoms[i0+1],space.atoms[i1+7])
        space.atoms2compute()
        pass
