...
def action1(space):
    ...
    if space.t==1:    # methyl + methyl
        ...
        i1 = space.merge_from_file("examples/nucleobase/adenine.json",60,60,0)
        bond_atoms(space.atoms[i0],space.atoms[i1])
        #space.atoms[i0+1].color = (0,1,0)
        #space.atoms[i1+6].color = (0,1,0)
        space.atoms[i0+1].nodes[1].q=0
        space.atoms[i1+6].nodes[2].q=0
        space.atoms2compute()
        index = 0

    if space.t==300:
        space.compute2atoms()
        bond_atoms(space.atoms[i0+1],space.atoms[i1+6])
        space.atoms2compute()
        pass
    ...
    if space.t==1000:
        ...
        space.atoms[33].color = (0,0,0)
        space.atoms2compute()
        pass

    # search water
    #if space.t>1 and space.t%400==0:
    #       print(index)
    #     space.compute2atoms()
