...
def action1(space):
    ...
    if space.t==2500: # +H
        space.compute2atoms()
        i1=space.merge_from_file("examples/simple/methyl.json",50+x,y,+30+z)
    ...
    if space.t==3000: # +H
        space.compute2atoms()
        i2=space.merge_from_file("examples/simple/aminogroup.json",x,50+y,z)
        i3=space.merge_from_file("examples/geterocyclic/indole.json",100+x,y,z)
    ...
    if space.t==3500: # +H
        ...
        space.atoms[i3+13].color = (0,0,0)
        space.atoms[i3+13].nodes[0].q=0
