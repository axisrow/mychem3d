...
def action1(space:Space):
    ...
    if space.t==600:
        space.compute2atoms()
        ami = space.merge_from_file("examples/simple/aminogroup.json",x,y+20,z-10)
        #space.appendatom(a1)
...
#        space.atoms2compute()
    ...
    if space.t==1400:
        space.compute2atoms()
        i4 = space.merge_from_file("examples/simple/carbonyl.json",x,y+40,z)
        bond_atoms(space.atoms[i4],space.atoms[i2])  #L/D
        space.atoms2compute()

    if space.t==2000:
        space.compute2atoms()
        i5 = space.merge_from_file("examples/simple/aminogroup.json",x+20,y+40,z)
